...
export function StepConverterPage() {
  ...
  const fileLabel = file
    ? file.name
    : t('tools.stepConverter.form.filePlaceholder');
  ...
  const handleSubmit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    if (mode === 'advanced' && hasInvalidNumbers) {
      setStatus({
        state: 'error',
    ...
    try {
      ...
      if (mode === 'advanced' && (includeBom || includeNodeMap)) {
    ...
    <div className="space-y-8">
      <header className="space-y-2">
        <div className="flex flex-wrap items-center gap-2">
          <Badge
            variant="outline"
            className="border-border/60 text-[11px] uppercase tracking-wide"
          >
          ...
          <CardContent>
            <form onSubmit={handleSubmit} className="space-y-6">
              ...
              <div className="flex flex-wrap items-center gap-3">
                ...
                {mode === 'advanced' && hasInvalidNumbers && (
              ...
              >
                <span className="inline-flex items-center gap-2 font-semibold">
                  ...
                  {status.state === 'idle' &&
                    t('tools.stepConverter.status.idle')}
                  {status.state === 'loading' &&
                    t('tools.stepConverter.status.loading')}
                  {status.state === 'success' &&
