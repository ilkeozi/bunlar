...
class RawWebApiEndpoint(Base):
    __tablename__ = "raw_web_api_endpoint"
    ...
    sample_request_url: Mapped[str] = mapped_column(String(2048), nullable=False, default="")
    payload_json: Mapped[str] = mapped_column(Text, nullable=False, default="{}")
    occurred_at: Mapped[datetime] = mapped_column(DateTime(timezone=True), nullable=False, server_default=func.now())
    created_at: Mapped[datetime] = mapped_column(DateTime(timezone=True), nullable=False, server_default=func.now())

    __table_args__ = (
        Index("ix_raw_web_api_endpoint_orch_id", "orchestration_id", "id"),
        Index("ix_raw_web_api_endpoint_batch_id", "ingest_batch_id", "id"),
        Index("ix_raw_web_api_endpoint_endpoint_url", "endpoint_url"),
    )
