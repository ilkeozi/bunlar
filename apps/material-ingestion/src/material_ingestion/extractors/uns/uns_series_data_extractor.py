...
class UnsSeriesDataExtractor:
    ...
    def _extract_from_table_rows(
        ...
        for raw_row in table_rows:
            ...
            if self._is_table_header_row(cells):
                continue
            if self._is_table_note_row(cells):
                continue
            ...
            if code_match:
                ...
                if expected_prefix and code[:1] != expected_prefix:
                    # Reject obvious bleed/noise rows (e.g. Cxxxxx inside Zxxxxx section pages).
                    continue
    ...
    @staticmethod
    def _is_table_note_row(cells: list[str]) -> bool:
        lowered = " ".join(cells).lower()
        note_phrases = (
            "the chemical compositions listed are for identification purposes",
            "the compositions listed are for identification purposes",
        ...
        if stripped in {"boxed", "'boxed", '"boxed', "* boxed", "* 'boxed", "'boxed of", "'boxed for"}:
            return True
        return False
