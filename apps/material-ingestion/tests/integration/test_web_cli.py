...
class WebCliIntegrationTest(unittest.TestCase):
    def test_discover_pdfs_command_persists_and_writes_output(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with (
                ...
                patch("material_ingestion.cli.WebPdfDiscovery") as mock_discovery_cls,
                patch("material_ingestion.cli.RawWebDbExporter") as mock_exporter_cls,
            ):
    ...
    def test_fetch_pdfs_command_downloads_and_persists(self) -> None:
        ...
        class _FakeSession:
            ...
            def query(self, model):
                ...
                if model is RawWebDownloadedFile.source_url:
                    return _FakeQuery([])
                return _FakeQuery([])
        ...
        with (
            ...
            patch("material_ingestion.cli.create_session_factory", return_value=_fake_session_factory),
            patch("material_ingestion.cli.WebFileDownloader") as mock_downloader_cls,
            patch("material_ingestion.cli.RawWebDbExporter") as mock_exporter_cls,
        ):
    ...
    def test_fetch_pdfs_uses_deepseek_for_ambiguous_candidates(self) -> None:
        ...
        with (
            ...
            patch("material_ingestion.cli.WebFileDownloader") as mock_downloader_cls,
            patch("material_ingestion.cli.RawWebDbExporter") as mock_exporter_cls,
            patch("material_ingestion.cli.DeepseekPdfClassifier") as mock_classifier_cls,
    ...
    def test_web_run_runs_discovery_then_fetch(self) -> None:
        ...
        query_rows = [_FakeCandidate("https://example.com/tds.pdf", 7, 1, "url_pdf_suffix")]
        ...
        def _fake_session_factory():
            return _FakeSession(query_rows)
        ...
        self.assertEqual(0, rc)
        self.assertEqual(1, mock_discovery.discover.call_count)
        self.assertEqual(1, mock_downloader.download_pdf.call_count)
